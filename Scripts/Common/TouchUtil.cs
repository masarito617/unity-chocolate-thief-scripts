...
namespace Chocolate.Common
{
    ...
    public static class TouchUtil
    {
        ...
        /// 画面がタッチされたかどうか？
        /// </summary>
        /// <param name="isTargetUIs">UIもタッチ対象とするか？</param>
        public static bool IsScreenTouch(bool isTargetUIs = false)
        {
            var touch = GetBeganTouch();
            if (touch.fingerId == -1) return false; // id=-1でない
            if (!isTargetUIs && IsPointerOver(touch)) return false; // UIと重なっていない
            return true;
        }
